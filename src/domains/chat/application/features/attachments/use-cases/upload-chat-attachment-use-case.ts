...
import { categorizeMimeType } from '@/utils/categorize-mime-type';
...
export class UploadChatAttachmentUseCase {
	...
	async execute({ fileName, fileType, roomId, messageId }: IRequest): Promise<Response> {
		const newAttachment = Attachment.create({
			...
			type: categorizeMimeType(fileType),
		});
